            ...
            <div class="horizontal-container">
              <div v-for="i in related.imgs" :key="i.id" class="horizontal-item">
                <nuxt-link :to="(i.illustID == result.illustID) ? '#' : `/arts/${i.illustID}`" :style="{ 'opacity': ( i.illustID == result.illustID ? 0.2 : 1.0) }">
                  <img class="vertical-centered" decoding="async" :src="`https://cdn.gochiusa.team/illusts/thumb/${i.illustID}.webp`">
                </nuxt-link>
              </div>
            </div>
          </div>
  ...
  methods: {
    ...
    async downloadImage () {
      ...
      if (!this.$device.isIos) {
        ...
        a.href = imageUrl
        a.download = `gochiira_${this.result.illustID}.${this.result.extension}`
        a.click()
        a.remove()
    ...
    async requestWaifu2x () {
      ...
      const resp = await this.$axios.get('https://api2.gochiusa.team/waifu2x', { params })
  ...
  head () {
    return {
      ...
      meta: [
        ...
        { hid: 'og:url', property: 'og:url', content: `https://illust.gochiusa.team/arts/${this.result.illustID}` },
